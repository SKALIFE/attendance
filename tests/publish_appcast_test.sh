...
EXPECTED_URL='https://github.com/SKALIFE/attendance/releases/download/v0.1.11/SKALA-Attendance-0.1.11-arm64.zip'
mkdir -p "$TEST_TMPDIR"
make_target
make_probe
make_raw_fetcher
...
make_candidate "$BAD_URL_CANDIDATE" 0.1.11 12 'https://example.invalid/SKALA-Attendance-0.1.11-arm64.zip'
expect_failure "$TEMP_DIR/bad-url.out" run_publisher "$BAD_URL_CANDIDATE"
assert_unchanged "$INITIAL_APPCAST" <(remote_file appcast.xml)

VALID_CANDIDATE="$TEMP_DIR/candidate.xml"
make_candidate "$VALID_CANDIDATE" 0.1.11 12 "$EXPECTED_URL"
...
assert_contains "$UPDATED_APPCAST" '<sparkle:version>12</sparkle:version>'
...
NEXT_URL='https://github.com/SKALIFE/attendance/releases/download/v0.1.12/SKALA-Attendance-0.1.12-arm64.zip'
NEXT_CANDIDATE="$TEMP_DIR/next-candidate.xml"
make_candidate "$NEXT_CANDIDATE" 0.1.12 13 "$NEXT_URL"
...
RAW_FAILURE_URL='https://github.com/SKALIFE/attendance/releases/download/v0.1.13/SKALA-Attendance-0.1.13-arm64.zip'
RAW_FAILURE_CANDIDATE="$TEMP_DIR/raw-failure-candidate.xml"
make_candidate "$RAW_FAILURE_CANDIDATE" 0.1.13 14 "$RAW_FAILURE_URL"
...
assert_contains <(remote_file appcast.xml) '<sparkle:version>14</sparkle:version>'

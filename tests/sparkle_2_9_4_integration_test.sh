...
GENERATOR_ARCHIVE="$TEMP_DIR/SKALA-Attendance-0.1.11-arm64.zip"
...
env SPARKLE_EDDSA_PRIVATE_KEY="$PRIVATE_KEY" bash "$ROOT/scripts/generate-appcast-item.sh" \
    --tag v0.1.11 --build 12 --archive "$GENERATOR_ARCHIVE" --archive-sha256 "$GENERATOR_DIGEST" \
    --archive-url 'https://github.com/SKALIFE/attendance/releases/download/v0.1.11/SKALA-Attendance-0.1.11-arm64.zip' \
